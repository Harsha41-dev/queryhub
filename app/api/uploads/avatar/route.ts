...
export async function POST(request: Request) {
  ...
  const limit = await checkRateLimit(`avatar:${session.user.id}`, 10, 60 * 60_000);
  ...
  let uploaded: Awaited<ReturnType<ReturnType<typeof objectStorage>["putAvatar"]>> | undefined;
  try {
    ...
    uploaded = await objectStorage().putAvatar(prepared.data);
    ...
    if (previous)
      void objectStorage()
        .deleteByUrl(previous.url)
        .catch((error) => captureException(error, { operation: "storage.avatar_cleanup" }));
    return NextResponse.json(
      actionSuccess({
        url: media.url,
        width: media.width,
